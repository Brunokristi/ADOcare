...
from models.exceptions.pathNotFoundException import PathNotFoundException
from models.exceptions.sizeMissMathException import SizeMissMathException
...
class tsp_solver:
    ...
    def __init__(self, config: Config = None, logger: Logger = None):
        if not hasattr(self, 'initialized'):
            ...
            self.ors_client = Client(base_url=config.getValue("open route", "link"))
    ...
    def calculate_part(self, start_location: List[float], points_locations: List[List[float]], end_location: List[float] = None):
        ...
        search_parameters.time_limit.seconds = self.config.getValue("open route", "optimization time limit")
