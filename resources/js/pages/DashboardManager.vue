...
<template>
  <div class="space-y-10 p-4">
    ...
    <section v-if="servicesDueThisMonth.length > 0" class="bg-warning p-6 rounded-md flex flex-col gap-4">
          ...
          <div class="space-y-2">
            <div v-for="service in servicesDueThisMonth" :key="service.id" class="bg-white p-3 rounded-md flex justify-between items-end">
              <div class="flex justify-between items-start">
                <div>
                    ...
                    <p class="text-normal text-darkgrey">{{ service.car?.user?.title}} {{ service.car?.user?.first_name }} {{ service.car?.user?.last_name }}</p>
