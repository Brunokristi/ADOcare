  ...
  <div class="flex flex-col gap-6">
    ...
    <div v-if="car.id">
        ...
        <div v-if="documentsPreviewUrls.length > 0" class="grid grid-cols-2 gap-3 bg-white p-3 rounded-md mb-4">
            ...
            <img v-if="url.startsWith('blob:')" :src="url" class="w-full h-40 object-cover rounded-md border border-gray-200" />
            <div v-else class="w-full h-40 rounded-md border border-gray-200 bg-gray-100 flex items-center justify-center text-gray-500">
                <i class="bi bi-file-pdf text-2xl"></i>
            </div>
            <button
                type="button"
                @click="removeDocumentPreview(index)"
        ...
        <div v-if="documents.length > 0" class="grid grid-cols-2 gap-3">
            ...
            :key="doc.id"
            class="relative cursor-pointer group rounded-md overflow-hidden"
            @click="openDocumentPreview(doc)"
            >
            <img 
                v-if="doc.mime_type?.startsWith('image/')"
                :src="getDocumentPreviewUrl(doc)"
                class="w-full h-40 object-cover bg-darkgrey/20"
                :alt="`Document ${doc.id}`"
            />
            <div 
                v-else
        ...
        <div class="flex justify-start items-center gap-2 mt-4">
            <Button icon="bi bi-plus-lg" @click="() => { if (!car.id) { toast.add({ severity: 'error', summary: 'Chyba', detail: 'Nejprve uložte auto', life: 3000 }); return } galleryInputRef?.click() }" class="bg-accent! border-none! hover:bg-darkgrey! h-7!" />
        </div>
    ...
    <div v-if="car.id">
        ...
        <Column header="">
            <template #body="{ data }">
            <Button icon="bi bi-pencil" class="text-darkgrey! text-normal! bg-transparent! border-none!" @click="editService(data)" />
            </template>
        </Column>
        <Column header="">
            <template #body="{ data }">
            <Button icon="bi bi-eraser" class="text-warning! text-normal! bg-transparent! border-none!" @click="deleteService(data)" />
            </template>
        </Column>
        </DataTable>

        <div class="flex justify-start items-center gap-2 mt-4">
            <Button icon="bi bi-plus-lg" @click="() => { editingServiceId = null; newService = { interval_days: 365, date: null }; showServiceDialog = true }" class="bg-accent! border-none! hover:bg-darkgrey! h-7!" />
        </div>
    ...
    <Dialog v-model:visible="showServiceDialog" :header="editingServiceId ? 'Upraviť' : 'Nová'" :modal="true" style="width: 500px">
      <div class="space-y-4">
        <div>
        ...
        <div>
          ...
          <Select 
            :modelValue="newService.interval_days" 
            @update:modelValue="(val) => newService.interval_days = val"
            :options="[
        ...
        <div class="flex gap-2 mt-4 justify-end">
          ...
          <Button label="Uložiť" class="bg-accent! border-accent! hover:bg-darkgrey! hover:border-darkgrey! px-2!" @click="saveService" />
        </div>
      </div>
    </Dialog>

    <Dialog v-model:visible="showDocumentPreview" :header="`Náhľad dokumentu`" :modal="true" style="width: 80vw; max-width: 900px">
      <div v-if="selectedDocumentPreview" class="flex flex-col gap-4">
        <img 
          v-if="selectedDocumentPreview.mime_type?.startsWith('image/')"
          :src="getDocumentPreviewUrl(selectedDocumentPreview)"
          class="w-full max-h-[70vh] object-contain rounded-md"
          :alt="selectedDocumentPreview.mime_type"
        />
        <!-- PDF or unsupported file -->
        <div v-else class="w-full h-[70vh] flex flex-col items-center justify-center bg-gray-100 rounded-md gap-4">
        ...
        <div class="flex gap-2">
          <Button 
            ...
            label="Vymazať" 
            severity="danger"
            class="flex-1 border-none! bg-warning! hover:bg-darkgrey!"
