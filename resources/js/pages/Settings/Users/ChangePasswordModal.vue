...
const save = async () => {
  ...
  saving.value = true
  try {
    await api.patch(`v1/users/${props.userId}`, { password: password.value })
    ...
    toast.add({ severity: 'error', summary: 'Chyba', detail: 'Nepodarilo sa zmeniť heslo', life: 3000 })
  } finally {
    saving.value = false
  }
}
...
<template>
  <div class="flex flex-col gap-4">
    <div>
      ...
      <small v-if="submitted && !password" class="text-warning">Heslo je povinné.</small>
    </div>

    <div>
      <label class="block text-sm mb-1">Potvrdite heslo</label>
      <IconField class="flex items-center w-full">
      ...
      <small v-if="submitted && !confirmPassword" class="text-warning">Potvrdenie hesla je povinné.</small>
