...
async function createScanSession() {
    ...
    try {        
        ...
        toast.add({
            severity: 'error',
            summary: 'Chyba',
            detail: error?.message || 'Chyba pri vytváraní QR kódu',
            life: 5000
        })
    } finally {
...
async function startPolling() {
    ...
    const pollInterval = setInterval(async () => {
        ...
        try {
            ...
            // Check if session has expired on backend
            if (statusData?.status === 'expired') {
                ...
                toast.add({
                    severity: 'warn',
                    summary: 'Upozornenie',
                    detail: 'Časový limit sessie vypršal',
                    life: 5000
                })
            ...
            // Check if document was completed
            ...
            // If backend returns 404 or session not found, session has expired
            if (error?.response?.status === 404) {
                ...
                toast.add({
                    severity: 'warn',
                    summary: 'Upozornenie',
                    detail: 'Čas vypršal pri čakaní na dokument',
                    life: 5000
                })
            ...
            // Frontend timeout as fallback (2 minutes)
            if (pollCount.value > 240) {
                ...
                toast.add({
                    severity: 'warn',
                    summary: 'Upozornenie',
                    detail: 'Timeout pri čakaní na dokument',
                    life: 5000
                })
...
/**
 * Open scan page in new tab for testing on desktop
 */
async function testScanPage() {
    try {
        if (!sessionId.value || !qrUrl.value) {
            toast.add({
                severity: 'warn',
                summary: 'Info',
                detail: 'Vytvoriť QR kód najprv',
                life: 3000
            })
            return
        }
        
        // Open the scan page in a new tab using the qrUrl (which is already a full URL)
        ...
        toast.add({
            severity: 'error',
            summary: 'Chyba',
            detail: 'Chyba pri otváraní testovacieho okna',
            life: 5000
        })
    }
}
...
watch(
    ...
    ({ patientId, branchId }) => {
        if (patientId && branchId && !sessionStarted.value) {
            console.log('Watch triggered for patientId:', patientId, 'branchId:', branchId)
...
<template>
    <div class="flex flex-col items-center justify-center gap-6 p-4">        
        <div v-if="isLoading" class="flex flex-col items-center gap-6">
            ...
            <p class="text-gray-600">Generujem QR kód...</p>
        </div>
        ...
        <div v-else-if="qrUrl" class="flex flex-col gap-4 w-full">
            <div class="flex flex-col items-center gap-4">
                <img 
                    ...
                    alt="QR Code for scan" 
                    class="w-40 h-40"
                />
            </div>

            <div class="">
                <p class="text-mini! text-darkgrey align-left">
                    <br>1. Otvorte aplikáciu fotoaparát na telefóne
                    <br>2. Naskenujte QR kód zobrazený vyššie
                    <br>3. Budete presmerovaný na stránku na nahratie dokumentu
                    <br>4. Odfoťte lekársky nález a nahrajte ho
                </p>
            </div>
            ...
            <div class="flex gap-2 w-full">
                <button 
                    @click="testScanPage"
                    class="flex-1 px-4 py-2 text-sm bg-blue-500 text-white rounded hover:bg-blue-600 transition"
                >
                    🔗 Testovať na tomto počítači
                </button>
            </div>

            <div v-if="isPolling" class="w-full flex flex-col items-center gap-4 py-6">
                <svg width="50" height="50" viewBox="0 0 237 100" xmlns="http://www.w3.org/2000/svg" class="logo-spinner">
                    <g class="orbit-left-spinner">
                        <path
                            d="M50 0C77.6142 0 100 22.3858 100 50C100 77.6142 77.6142 100 50 100C22.3858 100 0 77.6142 0 50C0 22.3858 22.3858 0 50 0ZM40.9062 36.0781V62H45.5312V57.6094L48.0469 55.3594L54.9062 62H61.8438L51.5938 52.1875L61.2344 43.5625H54.1562L45.5312 51.3906V36.0781H40.9062Z"
