...
const disabledDates = computed(() => {
    ...
    console.log('Calculating disabled dates, allowed:', allowedDaysInMonth.value)
...
async function fetchAllowedDays(keepDates = false) {
    ...
    try {
        ...
        console.log('API response for available dates:', res.data, res.data?.data?.days)
        ...
        console.log('Fetched allowed days for month', monthStartIso, ':', days)
        ...
        <form @submit.prevent="generateDekurz" class="flex flex-col gap-6">
            <section class="bg-tag3 p-6 rounded-md">
                <div class="grid grid-cols-2 gap-6">
                    <div>
                        ...
                        <DatePicker v-model="dekurzMonth" view="month" dateFormat="M yy" :showIcon="false"
                            class="w-full"
                            inputClass="!w-full !shadow-none !bg-white focus:!ring-0 focus:!shadow-none !border-0"
                            :maxDate="deathDate || undefined" :invalid="submitted && !!errors.dekurzMonth" />
                        <small v-if="submitted && errors.dekurzMonth" class="text-danger">{{ errors.dekurzMonth
                        }}</small>
                    </div>
            ...
            <div v-if="!timelineCalculated && !checkingTimeline && !calculationInProgress"
                ...
                <div>
                    ...
                    <p class="text-white text-mini">Zdá sa že časová os chýba. Spustite výpočet a pokračujte v úpravách
                        dekurzu.
                    </p>
                </div>
                <Button type="button" label="Spustiť výpočet"
                    class="bg-white! border-none! text-danger! h-7! px-2! shrink-0 text-normal!"
                    @click="recalculateTimeline" />
            </div>
            ...
            <section v-for="(section) in sections" :key="section.id" class="bg-tag3 p-6 rounded-md flex flex-col gap-4">
                <Toolbar class="bg-transparent! border-0! p-0! shadow-none! flex items-center justify-between no-print">
                    ...
                    <template #end>
                        <div class="flex items-center gap-2">
                            ...
                            <Button v-if="previousSectionTextById[section.id] !== undefined" type="button"
                                icon="bi bi-arrow-counterclockwise"
                                class="bg-accent! border-accent! text-white! hover:bg-grey! h-7!"
                                @click.prevent="restoreSectionText(section.id)" />

                            <Button icon="bi bi-eraser"
                                class="bg-danger! border-danger! hover:bg-darkgrey! hover:border-darkgrey! h-7!"
                ...
                <div class="w-full">
                    ...
                    <div v-if="!macrosLoading && !snippets.length" class="text-accent py-4 text-normal">
                        Nemáte žiadne makrá. Vytvorte si makrá pre rýchle vkladanie často používaných textov v <a
                            href="/settings/macros" target="_blank" class="underline">nastaveniach</a>.
                    </div>
            ...
            <div class="sticky bottom-0 z-20 flex justify-end">
                <div class="flex items-center gap-2">
                    <AdonisButton :loading="loadingAiPrefill" @click="prefillFromLatestProposalWithAi" />
                    <Button type="submit"
